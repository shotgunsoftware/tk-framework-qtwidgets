    ...
    def __on_worker_failure(self, uid, msg):
        ...
        if self._processing_id == uid:
            self._bundle.log_warning(
                "Could not retrieve activity stream " "data from Flow Production Tracking: %s" % msg
            )

        if uid in self._note_map:
            self._bundle.log_warning(
                "Could not retrieve note " "data from Flow Production Tracking: %s" % msg
            )

        if uid in self._thumb_map:
            # one of the jobs we are tracking
            self._bundle.log_warning(
                "Could not retrieve thumbnail " "data from Flow Production Tracking: %s" % msg
            )
